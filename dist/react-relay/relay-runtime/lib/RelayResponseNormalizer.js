...
function () {
  ...
  _proto._normalizeConnectionField = function _normalizeConnectionField(parent, selection, record, data) {
    ...
    var connectionRecordID = RelayModernRecord.getLinkedRecordID(record, storageKey);
    var connectionRecord = connectionRecordID != null ? this._recordSource.get(connectionRecordID) : null;
    ...
    var edgeIDs = RelayModernRecord.getLinkedRecordIDs(connectionRecord, EDGES);

    if (edgeIDs == null) {
      return;
    }

    var pageInfoID = RelayModernRecord.getLinkedRecordID(connectionRecord, PAGE_INFO);
  ...
  _proto._normalizeField = function _normalizeField(parent, selection, record, data) {
    ...
    } else if (selection.kind === LINKED_FIELD || selection.kind === CONNECTION_FIELD) {
